...
magnetar::Mesh::Mesh(GraphicsDevice *device, const std::vector<Vertex> &vertices, const std::vector<uint32_t> &indices)
    : m_vao(nullptr), m_vbo(nullptr), m_ibo(nullptr), m_index_count(indices.size())
{
    m_vao = device->create_vertex_array();
    m_vbo = device->create_vertex_buffer(vertices.size() * sizeof(Vertex), vertices.data());
    m_ibo = device->create_index_buffer(indices.size(), indices.data());
